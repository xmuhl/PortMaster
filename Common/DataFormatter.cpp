...
// 验证UTF-8字符序列的有效性（辅助函数） - KISS: 简化边界检查逻辑
bool DataFormatter::IsValidUtf8Sequence(const uint8_t* data, size_t maxLength)
{
    ...
    if ((firstByte & 0xE0) == 0xC0)      // 110xxxxx - 2字节字符
        ...
        byteCount = 4;
    else if ((firstByte & 0xC0) == 0x80) // 10xxxxxx - 后续字节，不能作为序列开始
        return false;  // 修复：后续字节不能单独存在
    else
        return false;  // 无效的首字节模式
        
    // KISS: 提前检查边界，避免后续访问越界
    if (maxLength < byteCount)
        return false;
        
    // 验证所有后续字节必须符合 10xxxxxx 格式
    ...
    // 额外验证：检查编码值是否在有效Unicode范围内
    if (byteCount == 2) {
        uint32_t codepoint = ((firstByte & 0x1F) << 6) | (data[1] & 0x3F);
        // 2字节UTF-8必须编码0x80-0x7FF范围的字符
        return codepoint >= 0x80 && codepoint <= 0x7FF;
    }
    else if (byteCount == 3) {
        uint32_t codepoint = ((firstByte & 0x0F) << 12) | ((data[1] & 0x3F) << 6) | (data[2] & 0x3F);
        // 3字节UTF-8必须编码0x800-0xFFFF范围的字符（排除代理区）
        return codepoint >= 0x800 && codepoint <= 0xFFFF && 
               !(codepoint >= 0xD800 && codepoint <= 0xDFFF);
    }
    else if (byteCount == 4) {
        uint32_t codepoint = ((firstByte & 0x07) << 18) | ((data[1] & 0x3F) << 12) | 
                           ((data[2] & 0x3F) << 6) | (data[3] & 0x3F);
        // 4字节UTF-8必须编码0x10000-0x10FFFF范围的字符
